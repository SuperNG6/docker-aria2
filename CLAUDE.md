...
**File filter (`DELETE_EXCLUDE_FILE`)** — *functionally identical*. Refactor pulled the 6 find pipes into a `_filter_rule` helper. Same FILE_NUM>1 guard, same `min-size/include-file/exclude-file/keyword-file/include-file-regex/exclude-file-regex` rules. Root-delete protection now sits in `REMOVE_SOURCE_PATH`/`MOVE_SOURCE_PATH` instead of being repeated in each filter function — functionally equivalent, less redundancy.
