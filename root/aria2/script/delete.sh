...
MOVE_FILE() {
    ...
    if [ ${MOVE_EXIT_CODE} -eq 0 ]; then
        ...
        [ $LOG_PATH ] && echo -e "$(date +"%m/%d %H:%M:%S") [INFO] delete files: ${SOURCE_PATH}" >>${LOG_PATH}
    else
...
RM_ARIA2() {
    if [ -e "${SOURCE_PATH}.aria2" ]; then
        echo -e "$(date +"%m/%d %H:%M:%S") ${INFO} Clean up ${SOURCE_PATH}.aria2"
...
if [ -e "${FILE_PATH}" ] && [ $2 -eq 1 ]; then # 普通单文件下载，直接删除
